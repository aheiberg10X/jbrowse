...
void store_indx(char *fname, Mates *mates, int n, long *strand_info, int strand_len, char *bam_file, char *chr, int chr_len){
	/*int i=0;
	printf("!%d\n!%d\n",n, ttl_grps);
...
int main(int argc, char **argv){
	if (argc!=6) ioerror("Usage: ./index_mates <mate_file> <bam_file> <output_file> <chr_name> <chr_length>");
	char *mate_file=argv[1];
	char *bam_file=argv[2];
	char *outpt_file=argv[3];
	char *chr_name=argv[4];
	int chr_len=atoi(argv[5]);
	...
	store_indx(outpt_file, read_info, ttl_reads, strand_info, strand_len, bam_file, chr_name, chr_len); //, grps, ttl_groups);
