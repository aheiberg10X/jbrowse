...
def print_body(l, code_type, src_table):
    ...
    if code_type=='read_filtering':
        ...
        if sp[3].count('interval_coverage')<=0:
            ioerror('Cannot find interval_coverage for interval filtering')
        cov=sp[5]
        comp=sp[4]
        if (comp[0]=='>'):
            up_low='0'
...
def call_imported_filtering(tbl_out, tbl_in, products_dir, back_end_dir, init_tbl, c_file):
    ...
    try: #check if valid_in exists
        ...
        valid_in=''
    print '%s/all_tools filter_imported %s %s %s'%(back_end_dir, tbl_indx, valid_dst, valid_in)
...
def call_filter_join(back_end_dir, products_dir, joined_table, tbl1, tbl2, tbl_out, read_indx, c_file):
    ...
    if type1=='reads': 
        ...
        init_tbl1=get_init_imported_name(tbl1, tables[joined_table].origin_path)
        indx1=products_dir+'/'+init_tbl1+'.table'
    ...
    if type2=='reads': 
        ...
        init_tbl2=get_init_imported_name(tbl2, tables[joined_table].origin_path)
        indx2=products_dir+'/'+init_tbl2+'.table'
...
def main(argv=sys.argv):
    if len(argv)!=10:
        ioerror("usage: python code_generator.py <bytecode_file> <products_dir> <front_end_dir> <back_end_dir> <src_table_dir> <bam_file> <indx_file> <chr_name> <chr_length> you gave: %s" % (str(argv)))
