...
sub a2a {
    ...
    if( ! defined $paired_info->{$qname} ){
        $paired_info->{$qname} = [$left,$right-1,$reversed,[$left,$right,$reversed,"hanging"]];
    }
    else {
        my $mates_info = $paired_info->{$qname};
        if( $mates_info->[0] < $left ){
            $mates_info->[3] = "left";
            $paired_info->{$qname} = [$mates_info->[0],$right-1,1,[$mates_info,[$left,$right,$reversed,"right"]]];
        }
        else{
            $mates_info->[3] = "right";
            $paired_info->{$qname} = [$left,$mates_info->[1]-1,1,[[$left,$right,$reversed,"left"],$mates_info]];
        }
