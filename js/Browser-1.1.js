...
Browser.prototype.createTrackList = function(parent, params) {
    ...
    // only use the tracks that pertain to this currently selected
    // reference sequence
    var current_refseq_name = brwsr.chromList.options[brwsr.chromList.selectedIndex].value;

    var filterByRefSeq = function(refseq) {
        var filterer = function(trackInfo) {
            return (trackInfo["refseq"] == refseq || trackInfo["refseq"] == "all");              
        };
        return filterer;
    };
  
    var filtered = params.trackData.filter( filterByRefSeq( brwsr.refSeq.name));
    this.trackListWidget.insertNodes( false, params.trackData );
    //this.trackListWidget.insertNodes(false, params.trackData.filter( filterByRefSeq( brwsr.refSeq.name) ));
